...
def change_column_types(tn, from_type, to_type):
    ...
    #
    # for c in tt.c:
    #     if c.name in ['tradeability_volume','tradeability_online']:
    #         if c.name == 'tradeability_volume':
    #             strSQL = f"alter table {tn} add constraint CONSTRAINT_2 check({c.name} in (0,1))"
    #         elif c.name ==  'tradeability_online':
    #             strSQL = f"alter table {tn} add constraint CONSTRAINT_3 check({c.name} in (0,1))"
    #         print(strSQL)
...
def main(argv):
    ...
    for tn in [t for t in instrument_names_here if t in Base.classes.keys() and not t.endswith("_tmp")]:
        ...
        # tt_from = meta.tables.get(tn)
        # tt_to = meta.tables.get(tn_bis)
        # if tt_to is not None:
        #     cs_from = [c.name for c in tt_from.c]
        #     cs_to = [c.name for c in tt_to.c]
        #     cs_to_copy = list(set(cs_from).intersection(set(cs_to)))
        #     cs_to_copy.remove('ID')
        #     strSQL = f"INSERT IGNORE INTO {tn_bis} ({', '.join(cs_to_copy)}) SELECT {', '.join(cs_to_copy)} FROM {tn}"
        #     print(strSQL)
        #     engine.execute(strSQL)
        #
        ...
        #     engine.execute(strSQL)
        #
        #     strSQL = f"RENAME TABLE {tn_bis} TO {tn}"
        #     print(strSQL)
        #     engine.execute(strSQL)
        #
        ...
        # add_column(tn,["timeslot","volume_in_timeslot","tradeability_volume","tradeability_online"],["DATETIME","BIGINT","TINYINT(1)","TINYINT(1)"])
        # update_metadata()
        # change_column_types(tn,sqlalchemy.dialects.mysql.TINYINT,sqlalchemy.BOOLEAN)
