...
def fillna_one_df(df, time_interval):
    ...
    # in contrast to numpy, the date_range function includes the "end" value; so we have to subtract time_interval
...
def update_original_table_driver(tn: str,
    ...
    # finally, we append this dataframe with missing timepoints to the original one and apply backfill
    # the result is a dataframe that has backfilled prices that are time_interval apart.
    ...
    if max_time > max_time_table:
        # we are forward filling, so now valid prices exist
